...
PVector lelbowPosition = new PVector();
PVector lshoulderPosition = new PVector();
...
void drawSkeleton(int userId){
 ...
 kinect.getJointPositionSkeleton(userId, SimpleOpenNI.SKEL_NECK,headPosition);
  // convert real world point to projective space
  kinect.convertRealWorldToProjective(headPosition,headPosition);
  // create a distance scalar related to the depth in z dimension
  distanceScalar = (300/headPosition.z);
  // draw the circle at the position of the head with the head size scaled by the distance scalar
  ellipse(headPosition.x,headPosition.y, distanceScalar*headSize,distanceScalar*headSize);
...
void keyPressed() {
 // Finishes the file
 userID = kinect.getUsers();
 if (key== 'r' && userID.length >0)
   {user = userID[0];
     if(kinect.isTrackingSkeleton(user))
       {
       kinect.getJointPositionSkeleton(user, SimpleOpenNI.SKEL_NECK,neckPosition);
       PrintWriter output;
       output = createWriter("angles.txt");
       output.println(str(neckPosition.x) + "," + str(neckPosition.y) + "," + str(neckPosition.z));
       
       kinect.getJointPositionSkeleton(user, SimpleOpenNI.SKEL_LEFT_SHOULDER,lshoulderPosition);
       output.println(str(lshoulderPosition.x) + "," + str(lshoulderPosition.y) + "," + str(lshoulderPosition.z));
      
       
       kinect.getJointPositionSkeleton(user, SimpleOpenNI.SKEL_LEFT_ELBOW,lelbowPosition);
       output.println(str(lelbowPosition.x) + "," + str(lelbowPosition.y) + "," + str(lelbowPosition.z));
       
       kinect.getJointPositionSkeleton(user, SimpleOpenNI.SKEL_LEFT_HAND,lelbowPosition);
       output.println(str(lelbowPosition.x) + "," + str(lelbowPosition.y) + "," + str(lelbowPosition.z));
       
        kinect.getJointPositionSkeleton(user, SimpleOpenNI.SKEL_RIGHT_SHOULDER,lshoulderPosition);
       output.println(str(lshoulderPosition.x) + "," + str(lshoulderPosition.y) + "," + str(lshoulderPosition.z));
      
       
       kinect.getJointPositionSkeleton(user, SimpleOpenNI.SKEL_RIGHT_ELBOW,lelbowPosition);
       output.println(str(lelbowPosition.x) + "," + str(lelbowPosition.y) + "," + str(lelbowPosition.z));
       
       kinect.getJointPositionSkeleton(user, SimpleOpenNI.SKEL_RIGHT_HAND,lelbowPosition);
       output.println(str(lelbowPosition.x) + "," + str(lelbowPosition.y) + "," + str(lelbowPosition.z));
       /*
       ...
       kinect.convertRealWorldToProjective(lshoulderPosition,lshoulderPosition);
       output.println(str(lshoulderPosition.x) + "," + str(lshoulderPosition.y) + "," + str(lshoulderPosition.z));
       
       kinect.convertRealWorldToProjective(lelbowPosition,lelbowPosition);
       output.println(str(lelbowPosition.x) + "," + str(lelbowPosition.y) + "," + str(lelbowPosition.z));
       */
